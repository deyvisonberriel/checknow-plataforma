...
class PagBankService {
   ...
   */
  async createPixOrder({ courseId, courseName, customer, amount = 2700 }) {
    ...
    if (!this.isConfigured()) {
      ...
      const mockPixCode = `00020126580014br.gov.bcb.pix0136checknow-sst@pix.pagbank.com.br520400005303986540527.005802BR5925CHECK NOW SST TREINAMENTOS6009SAO PAULO62070503***6304${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
      ...
      // Auto-aprovação em 5 segundos no modo Simulação / Mock para testes visuais completos
      setTimeout(() => {
        const order = this.memoryOrders.get(mockOrderId);
        if (order) {
          order.status = 'PAID';
          console.log(`[PagBank Mock] Pedido Pix ${mockOrderId} APROVADO AUTOMATICAMENTE após 5s para testes visuais!`);
          
          // Forçar envio de e-mail (Resend)
          console.log(`Tentando enviar e-mail via Resend para: ${customer.email}`);
          ...
          }).catch(err => console.error('[Resend Error Mock Callback]:', err));
        }
      }, 5000);
   ...
   */
  async createCardOrder({ courseId, courseName, customer, cardToken, amount = 2700, installments = 1 }) {
  ...
  async getOrderStatus(orderId) {
    if (!this.isConfigured() || orderId.startsWith('ORD-MOCK-')) {
      ...
      if (mock) {
        // Se já passaram 5 segundos desde a criação do pedido Pix, aprova automaticamente
        if (mock.status === 'PAID' || (mock.createdAt && Date.now() - mock.createdAt >= 5000)) {
          mock.status = 'PAID';
          status = 'PAID';
        }
      }
